...
__doc__="""
Benchmark 5a: Comparison of different collision models for domain with varying diffusion \
coefficient using Yantra's 3D implementation. The benchmark example refers to example of \
...
domain = yantra.Domain3D((0,0,0), (0.1,0.05,0.05),0.001, grid_type = 'midway',periodicity={'x':0,'y':1,'z':1})
x,_,_ = domain.meshgrid()
Dlow  = 1e-11
Dhigh = 2000 * Dlow
...
domain_params={}
domain_params['D']=D
domain_params['D0']=1e-11
domain_params['app_tort']=D/1e-11
domain_params['c']=0.1 
bc_params = {}
bc_params['left']=['c',0.]
bc_params['right']=['flux',0]       
solver_params={}         
solver_params['Dref']=Dlow/4
solver_params['Deref']=Dlow/4
...
ade_diff_vel=yantra.MultilevelAdvectionDiffusion(domain,domain_params,bc_params,solver_params)
solver_params['collision_model']='trt'
ade_trt=yantra.AdvectionDiffusion(domain,domain_params,bc_params,solver_params)
#%% run models
tf=500*3600*24 #500 days
ade_srt.run(time=tf)
...
plt.plot(x[z,y,:],ade_srt.c[z,y,:],label=ade_srt.collision_model.upper())
plt.plot(x[z,y,:],ade_trt.c[z,y,:],'-.',label=ade_trt.collision_model.upper())
plt.plot(x[z,y,:],ade_diff_vel.c[z,y,:],'--',label=ade_diff_vel.collision_model.upper())
plt.legend(loc=4)
plt.grid()
plt.axis('image')
