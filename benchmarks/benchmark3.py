...
__doc__="""
Benchmark 3: Comparision of 2D Yantra's diffusion implementation with \
...
domain = yantra.Domain2D((0,0),(200,20),1, grid_type = 'midway',periodicity={'x':0,'y':1})
#%% Define physics
cb = 1
D=0.1
#domain params 
...
solver_params['collision_model']='SRT' #other options 'trt'
solver_params['Dref']= domain_params['D']/10
solver_params['magic_para']= 1./4.

#create physics instance
diff = yantra.Diffusion(domain,domain_params,bc_params,solver_params)
#%%run model
diff.run(time=10000)    
#%%plot results   
plt.figure()
plt.contourf(diff.x,diff.y,diff.c)
plt.colorbar()
...
plt.plot(diff.x,diff.c[int(diff.ny/2),:],'--', label = 'Yantra')
plt.xlabel('distance (m)')
plt.ylabel(r'concentration (mol/m$^3$)')
plt.legend()
err = np.sqrt(np.sum((cany-diff.c[int(diff.ny/2),:])**2)/np.sum(cany**2))
