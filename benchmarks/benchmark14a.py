...
__doc__="""
Benchmark 14a: Poiseulle's flow by applying pressure gradient \
and zero velocity through boundary conditions on 3D geometry
"""
#%% import library
import sys,os
PARENT = '..'
...
plt.figure()
plt.contourf(x[loc_z,:,:],y[loc_z,:,:],ns.p[loc_z,:,:])
plt.axis('image')
...
p,u = analytic(pin,pout,ns.rho0,visc,x,y,l,l/2)
plt.figure()
...
plt.plot(x,ns.p[loc_z,loc_y,:],'o',x,p)
plt.legend(['LB','Analytical'])
plt.xlabel('distance')
plt.ylabel('pressure')
plt.show()

print('error(in %):', ns.l2norm(ns.u[0][loc_z,:,loc_x],u)*100) 
