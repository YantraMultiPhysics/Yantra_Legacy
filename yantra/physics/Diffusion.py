...
class Diffusion(AdvectionDiffusion, metaclass=LBMeta):
    """
    Solves diffusion equation written as
    ..math::
    ...
    \vec{\nabla}c  \cdot \hat{n} = 0 
    """
    #adjust list of variables removing unwanted ones
    _vars = _remove_keys(AdvectionDiffusion._vars,['u'])
    _signature = 'yantra.physics.Diffusion.Diffusion'
    ...
    def _construct_fort_solver(self,solver_params):
        ...
        if int(lattice[1]) == 2 and d == 2:
            ...
            solver_params['lattice']=lattice='D3Q7'
            solver_params['q']=7
        if lattice=='D3Q7':
            self._vars['es2'].default=1/3.5
        if d==2:
            diff = getattr(_solvers,'diff2d')
        elif d==3:
            diff = getattr(_solvers,'diff3d')   
        #compute_macro_var
        self.compute_macro_var = Fmethod(self,diff.compute_macro_var,
                                        ['_f', '_c', '_flux','nodetype','_tau'])
        #compute edf
        self.compute_edf = Fmethod(self,diff.compute_edf,
                                        ['_c','nodetype'],inplace_update=False)
        #collison
        if collision_model == 'srt':
            self.collide = Fmethod(self,diff.collide_srt,
                                   ['_f', '_c', 'nodetype','_tau','_ss'])
        elif collision_model == 'trt':
            self.collide = Fmethod(self,diff.collide_trt,
                                   ['_f', '_c','nodetype','_tau','_magic_para','_ss'])      
        else:
            raise ValueError('only SRT and TRT collision model available')
        #stream
        ...
        ['_f', 'nodetype', '_tau', '_interp','grid_type'],['_bc'])
